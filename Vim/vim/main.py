...
import mamba_ssm.ops.triton.layernorm as m_ln
...
def main(args):
    ...
    if args.eval:
        ...
        test_stats,init_entropy= evaluate(data_loader_val, model_ema.ema, device, amp_autocast, skip_blocks=[])
        skip_mamba_layers(model, replace_set=[])
...
def skip_mamba_layers(model, replace_set, verbose=True):
    replace_set = set(replace_set)
    for layer_idx, block in enumerate(model.layers):
        if not hasattr(block, "mixer"):
            continue
        if layer_idx in replace_set:
            block.mixer = Identity()

            if hasattr(block.mixer, "_orig_forward"):
                del block.mixer._orig_forward 

            if verbose:
                print(f"Permanently replaced the {layer_idx}-th Mamba layer (block.mixer) with Identity.")

        else:
            if verbose:
                print(f"Skipped layer {layer_idx}, kept its original Mamba implementation.")
