...
def get_args_parser():
    ...
    # Model parameters
    parser.add_argument('--model', default='EfficientViT_M5', type=str, metavar='MODEL',
...
def main(args):
    ...
    #replace_blocks_with_identity(model, [])
    replace_attention_with_identity(model,[])
    #replace_mixer_ffn_with_identity(model,[])
    model.to(device)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        test_stats,_ = evaluate(data_loader_val, model, device)
        print(
...
def replace_blocks_with_identity(model, skip_blocks):
    if not skip_blocks:
        print("No blocks specified to skip, the model remains unchanged.")
        return
        ...
        for local_id, blk in enumerate(stage):
            ...
            if isinstance(blk, EfficientViTBlock):
                if global_idx in skip_set:
                    print(f"Replacing the {local_id}-th EfficientViTBlock in Stage {stage_id} "
                        f"(global index {global_idx}) with Identity().")

                    stage[local_id] = nn.Identity()
                global_idx += 1

    print(f" A total of {len(skip_set)} blocks have been replaced with Identity.")
            ...
            if isinstance(blk, EfficientViTBlock):
                if global_idx in skip_set:
                    if hasattr(blk, "mixer") and hasattr(blk.mixer, "m"):
                        print(f"Replaced the Attention layer of the {local_id}-th EfficientViTBlock in Stage {stage_id+1} "
                            f"(global index {global_idx}) with Identity().")
                        blk.mixer.m = nn.Identity()  
                        replaced += 1
                    else:
                        print(f"Could not find the attention layer of block {global_idx}, skipping.")
                global_idx += 1

    print(f"A total of {replaced} attention layers have been replaced with Identity.")
                ...
                for ffn_name in ["ffn0", "ffn1"]:
                    if hasattr(blk, ffn_name):
                        if ffn_global_idx in skip_set:
                            ...
                            replaced += 1
                            print(f"Replaced {ffn_name} in FFN#{ffn_global_idx} → Block#{block_global_idx} "
                                f"(Stage {s_idx+1}, Local {l_idx}) with nn.Identity().")
                        ffn_global_idx += 1
                block_global_idx += 1

    print(f"A total of {replaced} FFNs have been replaced with Identity "
      f"(scanned {ffn_global_idx} FFN slots in total).")
