...
from model.efficientvit import EfficientViTBlock, PatchMerging
...
parser.add_argument('--model', '-m', metavar='NAME', default='EfficientViT_M5',
...
class BenchmarkRunner:
    def __init__(
        ...
        checkpoint = utils.load_model("/root/autodl-tmp/Cream/EfficientViT/efficientvit_m5.pth", model)
        ...
        replace_mixer_ffn_with_identity(model,[1,5,8,12])
        self.model = model
        ### Small
...
def replace_blocks_with_identity(model, skip_blocks):
    """
    将 EfficientViT 模型中指定的 EfficientViTBlock 替换为 nn.Identity()。

    参数:
        model: EfficientViT 实例
        skip_blocks: list[int]，要替换的全局 block 编号（例如 [0, 5, 8]）
    """
    if not skip_blocks:
        print("? 未指定要跳过的 block，模型保持不变。")
        return

    skip_set = set(int(i) for i in skip_blocks)
    stages = [model.blocks1, model.blocks2, model.blocks3]

    global_idx = 0  # 只统计 EfficientViTBlock
    for stage_id, stage in enumerate(stages):
        for local_id, blk in enumerate(stage):
            if isinstance(blk, nn.Sequential) or isinstance(blk, PatchMerging):
                # 跳过下采样或过渡层
                continue

            if isinstance(blk, EfficientViTBlock):
                if global_idx in skip_set:
                    print(f"?? 替换 Stage {stage_id} 的第 {local_id} 个 EfficientViTBlock "
                          f"(全局编号 {global_idx}) 为 Identity()")
                    stage[local_id] = nn.Identity()
                global_idx += 1

    print(f"? 共替换 {len(skip_set)} 个 block 为 Identity。")


def replace_attention_with_identity(model, skip_blocks):
    if not skip_blocks:
        print("⚠️ 未指定要跳过的 block，模型保持不变。")
        return

    skip_set = set(int(i) for i in skip_blocks)
    stages = [model.blocks1, model.blocks2, model.blocks3]

    global_idx = 0  # 只计数 EfficientViTBlock
    replaced = 0

    for stage_id, stage in enumerate(stages):
        for local_id, blk in enumerate(stage):
            if isinstance(blk, nn.Sequential) or isinstance(blk, PatchMerging):
                continue  # 跳过非主 block 结构

            if isinstance(blk, EfficientViTBlock):
                if global_idx in skip_set:
                    if hasattr(blk, "mixer") and hasattr(blk.mixer, "m"):
                        print(f"✅ 替换 Stage {stage_id+1} 的第 {local_id} 个 EfficientViTBlock "
                              f"(全局编号 {global_idx}) 的 Attention 层为 Identity()")
                        blk.mixer.m = nn.Identity()  # 注意：只替换 attention
                        replaced += 1
                    else:
                        print(f"⚠️ 找不到 block {global_idx} 的 attention 层，跳过。")
                global_idx += 1

    print(f"🎯 共替换 {replaced} 个 attention 层为 Identity。")


def replace_mixer_ffn_with_identity(model, skip_ffn_indices):

    if not skip_ffn_indices:
        print("⚠️ 未指定要替换的 FFN，全局索引列表为空。")
        return

    skip_set = set(int(i) for i in skip_ffn_indices)
    stages = [model.blocks1, model.blocks2, model.blocks3]
    ...
    PatchMerging = globals().get("PatchMerging", type("DummyPatchMerging", (), {}))

    for s_idx, stage in enumerate(stages):
        for l_idx, blk in enumerate(stage):
            # 跳过非主 block
            if isinstance(blk, (nn.Sequential, PatchMerging)):
                continue

            if isinstance(blk, EfficientViTBlock):
                for ffn_name in ["ffn0", "ffn1"]:
                    if hasattr(blk, ffn_name):
                        if ffn_global_idx in skip_set:
                            setattr(blk, ffn_name, nn.Identity())
                            replaced += 1
                            print(f"✅ 替换 FFN#{ffn_global_idx} → Block#{block_global_idx} "
                                  f"(Stage {s_idx+1}, Local {l_idx}) 的 {ffn_name} 为 nn.Identity()")
                        ffn_global_idx += 1
                block_global_idx += 1

    print(f"🎯 共替换 {replaced} 个 FFN 为 Identity（总计扫描 {ffn_global_idx} 个 FFN 槽位）")
