...
class EfficientViT(torch.nn.Module):
    ...
    def forward(self, x, skip_blocks=None):
        if skip_blocks is None:
            skip_blocks = set() 
        elif not isinstance(skip_blocks, set):
            skip_blocks = set(skip_blocks)

        x = self.patch_embed(x)
        stages = [self.blocks1, self.blocks2, self.blocks3]
        skip_set = set() if not skip_blocks else set(int(i) for i in skip_blocks)
        global_idx = 0
        global_block_idx = 0  


        for stage_idx, stage in enumerate(stages):
            for  local_idx, mod in enumerate(stage):
                if isinstance(mod, EfficientViTBlock):
                    if hasattr(mod, "mixer") and hasattr(mod.mixer, "m") and hasattr(mod.mixer.m, "attn"):
                        attn_layer = mod.mixer.m.attn
                        if not hasattr(attn_layer, "_orig_forward"):
                            attn_layer._orig_forward = attn_layer.forward

                        if global_block_idx in skip_set:
                            attn_layer.forward = lambda x: x 
                        else:
                            attn_layer.forward = attn_layer._orig_forward
                    x = mod(x)
                    global_block_idx += 1
                else:
                    x = mod(x)
